 ...
 * @param onDevHost — called with the generated discordId when a dev user logs in with host=true
 */
export function createAuthRouter(onDevHost?: (discordId: string) => void): Router {
  const router = Router();
  ...
  router.get("/dev", (req, res) => {
    ...
    // If this dev user is requesting host, notify the server
    if (req.query.host === "true" && onDevHost) {
      onDevHost(discordId);
    }
